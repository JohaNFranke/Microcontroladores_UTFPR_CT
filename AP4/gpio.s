...
; Definições dos registradores para ativação de interrupção
...
; Passo 1: Desabilitar GPIOIM de J0 e J1 (2 últimos bits)
	LDR 	R0, =GPIO_PORTJ_AHB_IM_R
	MOV 	R1, #2_00
	STR 	R1, [R0]
	; Passo 2: Configuração borda no registrador GPIOIS
	LDR 	R0, =GPIO_PORTJ_AHB_IS_R
	MOV 	R1, #2_00
	STR 	R1, [R0]
	; Passo 3a: Configuração borda única para os pinos J0 e J1 no registrador GPIOIBE
	LDR 	R0, =GPIO_PORTJ_AHB_IBE_R
	MOV 	R1, #2_00
	STR 	R1, [R0]
	; Passo 3b: Configuração borda de descida para J0 e subida para J1 no registrador GPIOIEV
	...
	MOV 	R1, #2_11
	STR 	R1, [R0]
	; Passo 5: Ativar interrupção nos pinos J0 e J1 em GPIOIM (desabilitada no passo 1)
	LDR 	R0, =GPIO_PORTJ_AHB_IM_R
	MOV 	R1, #2_11
	STR 	R1, [R0]
	; Passo 6: Ativar a fonte de interrupção no NVIC
	LDR 	R0, =NVIC_EN1_R
	MOV 	R1, #1
	LSL		R1, #19
	STR 	R1, [R0]
	; Passo 7: Configurar a prioridade da fonte de interrupção no NVIC (prioridade = 5)
	LDR 	R0, =NVIC_PRI12_R
	MOV 	R1, #5
	LSL 	R1, R1, #29
	STR 	R1, [R0]
